...
const CartProductComponent = () => {
    ...
    const handleAmount = (id, type) => {
        if (type === 'increase') {
            ...
            dispatch(descreaseAmount({ id }))
        }
    }
    const handleRemoveCart = (idProduc) => {
        dispatch(RemoveCart({ idProduc }))
    }
    ...
    useEffect(() => {
        dispatch(selecedItemOrder({ listChecked: checkedItems }))
    }, [checkedItems])

    const handleBuyCart =  () => {
        if (orderProduct?.selecedItemOrder.length === 0) {
            ...
            setInfoUser({
             ...
             orderProduct?.selecedItemOrder?.forEach(item => {
                if (item.amount > productRedux.countInStock) {
                    ...
                    <Row>
                        <Col md={8}>
                            ...
                            <Table striped bordered style={{ marginTop: '20px' }}>
                                    ...
                                    <tr>
                                        <th>
                                            <input type='checkbox' onChange={(e) => {
                                                if (e.target.checked) {
                                                    const allIds = orderProduct?.orderItems?.map(item => item.product) // map tạo thành một mảng các id
                                                    setCheckedItems(allIds)
                                                } else {
                                                    setCheckedItems([])
                                                }
                                            }}
                                                checked={orderProduct?.orderItems?.length > 0 && checkedItems.length === orderProduct?.orderItems?.length}
                                            />
                                        </th>
                                        <th>Tên sản phẩm</th>
                                        <th>Hình ảnh</th>
                                ...
                                <tbody >
                                    {orderProduct?.orderItems?.map(item => (
                                        <tr key={item.product}>
                                            <td>
                                                <input type='checkbox'
                                                    style={{
